...
export default function ProvinceDossier({
  ...
  return (
    <>
      ...
      <div
        className="sm:hidden fixed inset-0 bg-black/50 z-30 flex items-center justify-center"
        onClick={onClose}
      >
        {/* Stop propagation so clicking panel doesn't close */}
        <div onClick={(e) => e.stopPropagation()} />
      </div>

      {/* Panel — centered modal on mobile, fixed sidebar on desktop */}
      <div
        className="province-dossier-enter fixed z-30 hud-panel overflow-y-auto
                   inset-0 m-auto w-[85vw] max-h-[80vh] h-fit
                   sm:inset-auto sm:m-0 sm:top-12 sm:left-3 sm:w-80 sm:max-h-[calc(100vh-4rem)]"
